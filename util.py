...
def closest(closestList,mazeLength,gameData):
    minDist = mazeLength
    for k in closestList:
        if(abs(gameData.posPacman-k)<=minDist):
            minDist = abs(gameData.posPacman-k)
    return minDist

def closestList(closestList,mazeLength,gameData):
    minDistList = []
    returnList = []
    minDist = mazeLength
    count = 0
...
def base10toN(num, n, length): #copied from http://code.activestate.com/recipes/65212-convert-from-decimal-to-any-base-number/
    ...
    while current!=0:
        ...
        current=current/n
    for k in range(0, length-len(new_num_string)):
